function [A, b, x, ProbInfo] = PRtomo_mismatched(varargin) 
...
%    phantomImage - character string indicating the type of image
...
%%%% MALENA: CHECK THIS ONE PROPERLY
%   isMatrix - Choose the operator type:
%             'on' for a cell with two matrices as an output
%             'off' for a function handle as an output
%
% Output:   
...
switch CTtype
    ...
    case 'fancurved'
        ...
        %A = fancurvedtomo_mismatched(N,angles,p,R,span,0,sm);
        A = fancurvedtomo_mismatched(N,angles,p,R,span,0,sm);
   otherwise
